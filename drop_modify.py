...
# images
drop_txt = pygame.image.load("images/drop_d.png")
...
skin_btn_img = pygame.image.load("images/btn_skins.png")
...
cant_buy = pygame.image.load("images/btn1000_gray.png")
...
def loading():
    load_img = pygame.image.load("images/loading.png")
    running = True
    cnt = 0
    alpha = 3
    opacity = 255
...
def menu(id):
    ...
    pix_dead_img = pygame.image.load(all_characters[id-1]["kill_32"])
    pix_dead_img_big = pygame.image.load(all_characters[id-1]["kill_64"])
    start_btn = pygame.image.load("images/play_btn.png")
    x = 0
    dx = 1
    running = True
    click = False
    while running:
        ...
        btn = pygame.Rect(size[0]//2-start_btn.get_width()//2 + x,
                          size[1] // 3 + pix_Img.get_height() + 50,
                          start_btn.get_width(),
                          start_btn.get_height()
                          )
        skin_btn = pygame.Rect(size[0]//2-skin_btn_img.get_width()//2, size[1]//3 + 300,
        ...
        screen.blit(drop_txt, (size[0]//2 - drop_txt.get_width()//2, 100))
        screen.blit(best_score_img, (size[0]//2-best_score_img.get_width() //
                                     2 + x, size[1]//3 - best_score_img.get_height() + 45))
        best_score = best_score_font.render(
            f"{user_best_score}", False, (255, 140, 16))
        screen.blit(best_score,
                    (size[0]//2 - best_score.get_width()//2 + x, size[1]//3 - best_score_img.get_height()+50))
        screen.blit(
            pix_Img, (size[0]//2-pix_Img.get_width()//2 + x, size[1]//3+50))
        screen.blit(start_btn, (size[0]//2-start_btn.get_width() //
                                2 + x, size[1]//3 + pix_Img.get_height()+50))
        screen.blit(
            skin_btn_img, (size[0]//2-skin_btn_img.get_width()//2, size[1]//3 + 300))
        pygame.display.flip()
        clock.tick(fps)
...
def game(pix_Img, pix_Img_big, user_score, platforms, enemys, start=False, balls=[], pix_dead_img=None, pix_dead_img_big=None):
    ...
    while running:
        ...
        if col_r:
            ...
            if 'mushroom' in bonuses:
                my_pix.image = pygame.transform.rotate(pix_dead_img_big, spin)
            else:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    running = False
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1 and clickable:
                    click = True
        ...
        if len(balls) == 3 and len(set(balls)) == 1:
            ...
            bonus_raund(pix_Img, pix_Img_big, user_score-1, balls[0], pix_dead_img, pix_dead_img_big)
            running = False
        for i in range(3):
            try:
...
def restart(pix, pix_Img_big, score, pix_dead_img, pix_dead_img_big):
    global user_best_score, user_stars
    running = True
    while running:
        background(user_stars)
...
def continue_game(pix, pix_Img_big, score, platforms, enemys, balls, pix_dead_img, pix_dead_img_big):
    global user_best_score, user_stars
    running = True
    while running:
        background(user_stars)
...
def bonus_raund(pix, pix_big, user_score, color, pix_dead_img, pix_dead_img_big):
    global user_stars
