...
def menu(): 
    ...
    while True:
        ...
        btn = pygame.Rect(size[0]//2-start.get_width()//2 + x, size[1]//3 + pix_size, start.get_width(), start.get_height())
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    click = True
        screen.blit(pix_Img, (size[0]//2-pix_size//2 + x , size[1]//3))
        screen.blit(start, (size[0]//2-start.get_width()//2 + x, size[1]//3 + pix_size))
        pygame.display.flip()
        clock.tick(fps)
